# import numpy as np
from fractions import Fraction
import math
import sys
sys.stdout.reconfigure(encoding='utf-8')


def invert_matrix(mat):
    """Получает матрицу -> возвращает обратную"""
    n = len(mat)
    # Augment with identity
    aug = [row[:] + [Fraction(int(i==j)) for j in range(n)] for i,row in enumerate(mat)]
    for i in range(n):
        # Pivot
        if aug[i][i] == 0:
            for r in range(i+1, n):
                if aug[r][i] != 0:
                    aug[i], aug[r] = aug[r], aug[i]
                    break
        piv = aug[i][i]
        if piv == 0:
            raise RuntimeError("Singular matrix")
        for j in range(2*n):
            aug[i][j] /= piv
        for r in range(n):
            if r != i:
                factor = aug[r][i]
                if factor != 0:
                    for j in range(i, 2*n):
                        aug[r][j] -= factor * aug[i][j]
    inv = [[aug[i][j] for j in range(n,2*n)] for i in range(n)]
    return inv


# Класс, который хранит матрицу A, b, b' и С и проводит с ней вычесления 
class ParametricSimplex:
    """Решает параметрическую злп"""
    def __init__(self, A, b_prime, b_double, c):
        # Хранит оригинальные данные без вычеслений
        self.orig_A = [[Fraction(x) for x in row] for row in A]
        self.orig_b_prime = [Fraction(x) for x in b_prime]
        self.orig_b_double = [Fraction(x) for x in b_double]
        self.orig_c = [Fraction(x) for x in c]

        self._setup(self.orig_A, self.orig_b_prime, self.orig_b_double, self.orig_c)



    def _setup(self, A, b_prime, b_double, c):
        """Set up augmented matrix with slack variables and initial basis."""
        self.A = [row[:] for row in A]
        self.b_prime = b_prime[:]
        self.b_double = b_double[:]
        self.c = c[:]  # objective coefficients
        self.m = len(self.A)
        self.n = len(self.A[0]) if self.m>0 else 0
        # Add slack columns (identity matrix) to A, and zero cost for them
        for i in range(self.m):
            self.A[i].extend(Fraction(1) if j==i else Fraction(0) for j in range(self.m))
        self.c.extend(Fraction(0) for _ in range(self.m))
        self.total_vars = self.n + self.m
        # Initial basis: slack variables (indices n..n+m-1)
        self.basis = list(range(self.n, self.n+self.m))
        self.nonbasis = list(range(self.n))
        self._update_B_inv()



    def _update_B_inv(self):
        """обновляет вектор B для матрицу текущего базиса (на основе формулы Шермана-Моррисона как то так хз)"""
        B_cols = []
        for bi in self.basis:
            col = [self.A[i][bi] for i in range(self.m)]
            B_cols.append(col)
        B = [list(row) for row in zip(*B_cols)]
        self.B_inv = invert_matrix(B)


    def _simplex(self, b_const):
        """
        СИмплекс метод. обычный шаг. Принимает текущую матрицу  и в векторы и возвращает обновленный базис после одного или нескольких шагов
        """
        for _ in range(1000):
            # Compute basic solution x_B = B_inv * b_const
            xB = [sum(self.B_inv[i][k]*b_const[k] for k in range(self.m)) for i in range(self.m)]
            # Compute reduced costs using Bland's rule
            cB = [self.c[j] for j in self.basis]
            # y = c_B^T * B_inv
            y = [sum(cB[k]*self.B_inv[k][i] for k in range(self.m)) for i in range(self.m)]
            enter = None
            # Choose smallest-index nonbasic with positive reduced cost
            for j in sorted(self.nonbasis):
                Aj = [self.A[i][j] for i in range(self.m)]
                reduced = self.c[j] - sum(y[k]*Aj[k] for k in range(self.m))
                if reduced > 0:
                    enter = j
                    break
            # If no entering, optimum reached
            if enter is None:
                return
            # Compute pivot column B_inv * A[:,enter]
            Aj = [self.A[i][enter] for i in range(self.m)]
            col = [sum(self.B_inv[i][k]*Aj[k] for k in range(self.m)) for i in range(self.m)]
            # Ratio test for leaving variable
            min_ratio = None
            leave_idx = None
            for i in range(self.m):
                if col[i] > 0:
                    ratio = xB[i] / col[i]
                    if leave_idx is None or ratio < min_ratio or (ratio == min_ratio and self.basis[i] < self.basis[leave_idx]):
                        min_ratio = ratio
                        leave_idx = i
            if leave_idx is None:
                raise RuntimeError("Unbounded solution during simplex")
            # Pivot (Bland): replace basis
            leaving_var = self.basis[leave_idx]
            self.basis[leave_idx] = enter
            self.nonbasis.remove(enter)
            self.nonbasis.append(leaving_var)
            self._update_B_inv()
        raise RuntimeError("Simplex did not terminate")

    def _compute_interval(self):
        """
        Определяем интервал значений лямбды
        """
        alpha = [sum(self.B_inv[i][k]*self.b_prime[k] for k in range(self.m)) for i in range(self.m)]
        beta  = [sum(self.B_inv[i][k]*self.b_double[k] for k in range(self.m)) for i in range(self.m)]
        L, U = -math.inf, math.inf
        for i in range(self.m):
            if beta[i] > 0:
                val = -alpha[i]/beta[i]
                if val > L:
                    L = val
            elif beta[i] < 0:
                val = -alpha[i]/beta[i]
                if val < U:
                    U = val
            else:
                if alpha[i] < 0:
                    return None  # infeasible
        return (L, U, alpha, beta)

    def _dual_enter(self, leave_idx):
        """
        Dual simplex rule: given a leaving row index, pick smallest-index nonbasic
        with a negative entry in that row of B_inv * A.
        """
        row = [sum(self.B_inv[leave_idx][k] * self.A[k][j] for k in range(self.m)) 
               for j in range(self.total_vars)]
        candidates = [j for j in self.nonbasis if row[j] < 0]
        return None if not candidates else min(candidates)

    def solve(self):
        """
            Главный код, итератором находим решение для текущей гаммы, вычисляем интервал и переходит к другому
        """
        # Find initial BFS (choose a feasible lambda, e.g. lambda_0)
        lam0 = self._find_initial_lambda()
        # Perform standard simplex at lambda_0 to get optimal basis
        b0 = [self.b_prime[i] + lam0*self.b_double[i] for i in range(self.m)]
        self._simplex(b0)
        intervals = []
        # Record first interval
        iv = self._compute_interval()
        if iv is None:
            return intervals
        L, U, alpha, beta = iv
        intervals.append((L, U, alpha, beta, self.basis.copy()))
        # Move forward in lambda (increasing)
        while True:
            iv = self._compute_interval()
            if iv is None:
                break
            L, U, alpha, beta = iv
            if U == math.inf:
                break
            # Find which variable hits zero at U
            leave_idx = None
            for i in range(self.m):
                if beta[i] < 0 and -alpha[i]/beta[i] == U:
                    leave_idx = i
                    break
            if leave_idx is None:
                break
            enter = self._dual_enter(leave_idx)
            if enter is None:
                break
            leaving_var = self.basis[leave_idx]
            self.basis[leave_idx] = enter
            self.nonbasis.remove(enter)
            self.nonbasis.append(leaving_var)
            self._update_B_inv()
            iv2 = self._compute_interval()
            if iv2 is None:
                break
            L2, U2, alpha, beta = iv2
            intervals.append((U, U2, alpha, beta, self.basis.copy()))
        # Move backward in lambda (decreasing) by re-solving from lam0
        self._setup(self.orig_A, self.orig_b_prime, self.orig_b_double, self.orig_c)
        lam0 = self._find_initial_lambda()
        b0 = [self.b_prime[i] + lam0*self.b_double[i] for i in range(self.m)]
        self._simplex(b0)
        iv = self._compute_interval()
        if iv is None:
            return intervals
        # Record first interval again (skip, already in list)
        while True:
            iv = self._compute_interval()
            if iv is None:
                break
            L, U, alpha, beta = iv
            if L == -math.inf:
                break
            leave_idx = None
            for i in range(self.m):
                if beta[i] > 0 and -alpha[i]/beta[i] == L:
                    leave_idx = i
                    break
            if leave_idx is None:
                break
            enter = self._dual_enter(leave_idx)
            if enter is None:
                break
            leaving_var = self.basis[leave_idx]
            self.basis[leave_idx] = enter
            self.nonbasis.remove(enter)
            self.nonbasis.append(leaving_var)
            self._update_B_inv()
            iv2 = self._compute_interval()
            if iv2 is None:
                break
            L2, U2, alpha, beta = iv2
            intervals.insert(0, (L2, L, alpha, beta, self.basis.copy()))
        return intervals

    def _find_initial_lambda(self):
        """
        Find a feasible lambda for the initial slack basis (or nearest boundary).
        """
        alpha = self.b_prime[:]
        beta  = self.b_double[:]
        lam_min, lam_max = -math.inf, math.inf
        for i in range(self.m):
            if beta[i] > 0:
                val = -alpha[i]/beta[i]
                lam_min = max(lam_min, val)
            elif beta[i] < 0:
                val = -alpha[i]/beta[i]
                lam_max = min(lam_max, val)
            else:
                if alpha[i] < 0:
                    raise RuntimeError("No feasible region for initial slack basis")
        if lam_min == -math.inf and lam_max == math.inf:
            return Fraction(0)
        if lam_min == -math.inf:
            return lam_max
        if lam_max == math.inf:
            return lam_min
        # Choose 0 if inside, else use lam_min
        return Fraction(0) if lam_min <= 0 <= lam_max else lam_min

    
c = [3, 0, 0, -3, 2] 
A = [
    [0, 1, 0, 3, -10],   
    [1, 0, 0, -1, 13],
    [0, 0, 1, 7, 6]    
]
b_prime = [27, 117, 151]     
b_double = [-18, 18, -18]  

ps = ParametricSimplex(A, b_prime, b_double, c)
intervals = ps.solve()


# print(intervals)
# Print results
if intervals:
    # Проверка: нет решений до первого интервала
    if intervals[0][0] != -math.inf:
        print(f"[-\u221E, {intervals[0][0]}): решений нет\n")

    for (L, U, alpha, beta, basis) in intervals:
        # Вычисляем коэффициенты функции цели: F = c0 + c1*λ
        c0 = Fraction(0)
        c1 = Fraction(0)
        for i, var in enumerate(basis):
            if var < len(ps.orig_c):
                c0 += ps.orig_c[var] * alpha[i]
                c1 += ps.orig_c[var] * beta[i]
        # Формируем решение X
        X_expr = []
        for j in range(len(ps.orig_c)):
            if j in basis:
                idx = basis.index(j)
                a_j = alpha[idx]
                b_j = beta[idx]
            else:
                a_j = Fraction(0)
                b_j = Fraction(0)
            if b_j >= 0:
                X_expr.append(f"{a_j} + {b_j}*λ" if b_j != 0 else f"{a_j}")
            else:
                X_expr.append(f"{a_j} - {-b_j}*λ")

        # Строковые представления границ
        L_str = "-\u221E" if L == -math.inf else str(L)
        U_str = "\u221E" if U == math.inf else str(U)

        print(f"[{L_str}, {U_str}]")
        print(f"F = {c0}" + (f" + {c1}*λ" if c1 >= 0 else f" - {-c1}*λ"))
        print("X = [" + ", ".join(X_expr) + "]\n")

    # Проверка: нет решений после последнего интервала
    if intervals[-1][1] != math.inf:
        print(f"({intervals[-1][1]}, \u221E]: решений нет\n")
